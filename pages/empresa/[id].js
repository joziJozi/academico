...
const form = () => {
  ...
  useEffect(() => {
    if (query.id) {
      ...
      const empresa = empresas[query.id]
      for(let atributo in empresa){
        setValue(atributo, empresa[atributo])
      }

      setValue('razao social', empresa.razaosocial)
  ...
  function salvar(dados) {
    const empresas = JSON.parse(window.localStorage.getItem('empresas')) || []
    empresas.splice(query.id, 1, dados)
